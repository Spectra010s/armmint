...
export async function claimNextDueMintJob(
  ...
  return db.transaction(async (tx) => {
    const candidate = await tx.execute<{ id: string }>(sql`
      SELECT ${mintJobs.id} AS id
      FROM ${mintJobs}
      WHERE
        (${mintJobs.state} = 'SCHEDULED' AND ${mintJobs.scheduledFor} <= ${now})
        OR
        (
          ${mintJobs.state} = 'CLAIMED'
          AND ${mintJobs.claimExpiresAt} IS NOT NULL
          AND ${mintJobs.claimExpiresAt} <= ${now}
        )
      ORDER BY ${mintJobs.scheduledFor} ASC, ${mintJobs.createdAt} ASC
