...
Before signing, the engine commits an immutable public request (destination, calldata, value, gas and fees) and nonce. A wallet row lock serializes nonce allocation across jobs, and a job execution lease suppresses duplicate executors. All retries of an existing request reuse its stored nonce and inputs. Before broadcasting, the signing adapter commits the locally calculated transaction hash. The provider's returned hash must match it.
